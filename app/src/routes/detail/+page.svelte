<script lang="ts">
  ...
  let referrer: string = "/" // 기본 referrer 값은 홈
  ...
  // 뒤로가기 함수를 goto로 변경
  function goBack() {
    console.log(`[Detail] goBack: 이전 페이지로 이동합니다. referrer: ${referrer}`)
    goto(referrer)
  }
  ...
  onMount(() => {
    ...
    // referrer 파라미터 읽기 - 이전 페이지 정보 저장
    const refParam = params.get("referrer")
    if (refParam) {
      referrer = refParam
    } else {
      // document.referrer가 있으면 사용, 없으면 모드에 따라 기본값 설정
      referrer = mode === "edit" ? "/Installed-MCP" : "/MCP-list"
    }
    console.log(`[Detail] 이전 페이지 경로: ${referrer}`)
...
<div class="p-8 max-w-5xl mx-auto">
  <div class="bg-white rounded-lg shadow-sm p-6 relative">
    ...
    <div class="mb-8 border-b pb-4">
      <div class="flex items-center gap-3">
        ...
        {#if url && url.includes("github.com")}
          <button on:click={() => openGitHub(url)} class="text-gray-500 hover:text-gray-700 transition-colors" title="GitHub 리포지토리 열기">
            <Github size={20} />
          </button>
        {/if}
      </div>
    ...
    <div class="mt-8 flex justify-end gap-2 border-t pt-4">
      <button class="btn btn-sm btn-outline" on:click={goBack}>취소</button>
      <button
        ...
        on:click={() => {
          if (mode === "edit") {
            ...
            invoke("add_mcp_server_config", {
              ...
              .catch((err) => {
                alert(`MCP 설치 중 오류가 발생했습니다: ${err}`)
                console.error("MCP 설치 오류:", err)
              })
          }
        }}
